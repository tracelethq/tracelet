...
import { CalendarIcon, CornerDownLeftIcon, FileIcon, PlusIcon, Trash2Icon } from "lucide-react"
...
const ROW_TYPES = ["String", "Number", "Boolean", "Date", "DateTime", "File"] as const
...
function timePartTo12h(hhmm: string): { hour12: string; minute: string; ampm: "AM" | "PM" } {
  const [h, m] = hhmm.split(":")
...
export function ParamsTable({
  ...
  const handleFileChange = (id: string, e: React.ChangeEvent<HTMLInputElement>, row: ParamRow) => {
  ...
  const removeFileAt = (rowId: string, index: number) => {
    ...
    if (max <= 1) {
      ...
      updateRow(rowId, { files: next, value: next.length ? next.map((f) => f.name).join(", ") : "" })
    }
  }
                      ...
                      return (
                        <label className="flex cursor-pointer items-center gap-2">
                          <input
                            ...
                            accept={accept(row)}
                            onChange={(e) => handleFileChange(row.id, e, row)}
                                ...
                                {fileList.map((file, idx) => (
                                  ...
                                  >
                                    ...
                                    <Button
                                      ...
                                      onClick={() => removeFileAt(row.id, idx)}
                  ...
                  <Popover>
                    ...
                    <PopoverContent align="start" className="w-auto p-0">
                      <Calendar
                        ...
                        onSelect={(d) => {
                          if (d) updateRow(row.id, { value: format(d, "yyyy-MM-dd") })
                        }}
                        initialFocus
                      />
                    </PopoverContent>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {row.value
                            ? (() => {
                                const [datePart, timePart] = row.value.split("T")
                                try {
                                  const d = datePart ? parseISO(datePart) : null
                                  const dateStr = d && !isNaN(d.getTime()) ? format(d, "PPP") : ""
                                  const timeStr = timePart ? formatTime12h(timePart.slice(0, 5)) : ""
                                  return timeStr ? `${dateStr} ${timeStr}` : dateStr || "Pick date & time"
                                } catch {
                      ...
                      <PopoverContent align="start" className="w-auto p-0">
                        <Calendar
                          ...
                          onSelect={(d) => {
                            if (d) {
                              const timePart = row.value?.includes("T") ? row.value.split("T")[1]?.slice(0, 5) ?? "00:00" : "00:00"
                              updateRow(row.id, { value: `${format(d, "yyyy-MM-dd")}T${timePart}` })
                            }
                          }}
                        ...
                        <div className="border-t border-border p-2">
                          <div className="text-muted-foreground flex items-center gap-2 text-xs">
                            ...
                            {(() => {
                              const timePart = row.value?.includes("T") ? row.value.split("T")[1]?.slice(0, 5) : "00:00"
                              const { hour12, minute, ampm } = timePartTo12h(timePart)
                              const datePart = row.value?.includes("T") ? row.value.split("T")[0] : format(new Date(), "yyyy-MM-dd")
                              const setTime = (h: string, m: string, a: string) =>
                                updateRow(row.id, { value: `${datePart}T${to24h(h, m, a)}` })
                              return (
                                <div className="flex items-center gap-1.5">
                                  <Select
                                    value={hour12}
                                    onValueChange={(h) => setTime(h, minute, ampm)}
                                  >
                                    <SelectTrigger size="sm" className="h-7 w-12 font-mono text-xs">
                                  ...
                                  <Select
                                    value={minute}
                                    onValueChange={(m) => setTime(hour12, m, ampm)}
                                  >
                                    <SelectTrigger size="sm" className="h-7 w-14 font-mono text-xs">
                                  ...
                                  <Select
                                    value={ampm}
                                    onValueChange={(a) => setTime(hour12, minute, a)}
                                  >
                                    <SelectTrigger size="sm" className="h-7 w-14 font-mono text-xs">
                  ...
                  {allowTypeChange ? (
                    <select
                      ...
                      onChange={(e) =>
                        updateRow(row.id, {
                          type: e.target.value,
                          ...(e.target.value !== "File" ? { file: undefined, files: undefined } : {}),
                        })
                      }
