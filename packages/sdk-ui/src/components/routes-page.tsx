...
export function RoutesPage() {
  ...
  const [selectedRoute, setSelectedRoute] = React.useState<RouteMeta | null>(null)
  ...
  const fetchRoutes = React.useCallback(async () => {
    ...
    try {
      ...
      const urlRouteKeyDecoded = urlRouteKey ? decodeURIComponent(urlRouteKey) : null
      const fromUrl =
        urlRouteKeyDecoded &&
        flat.find((r: RouteMeta) => `${r.method}:${r.path}` === urlRouteKeyDecoded)
      setSelectedRoute((prev) => {
        if (fromUrl) return fromUrl
        const match =
          prev && flat.find((r: RouteMeta) => r.path === prev.path && r.method === prev.method)
  ...
  React.useEffect(() => {
    ...
    if (!exists) {
      setRouteInUrl(selectedRoute ? `${selectedRoute.method}:${selectedRoute.path}` : null)
    }
      ...
      <SidebarInset>
        ...
        <div className="relative flex flex-1 flex-col overflow-hidden">
          ...
          {!loading && !error && !hasRoutes && (
            <div className="flex flex-1 items-center justify-center p-4">
              <div className="w-full max-w-md">
                <EmptyRoutesState onRefresh={fetchRoutes} />
              </div>
            </div>
          )}
          ...
          {!error && hasRoutes && selectedRoute && (
            <ApiDetailsPanel
              {...({ route: selectedRoute, apiBase } satisfies ApiDetailsPanelProps)}
            />
          )}
          ...
          {loading && hasRoutes && selectedRoute && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/50">
              <div className="flex flex-col items-center gap-2 rounded-lg border bg-card px-4 py-3 shadow-sm">
                ...
                <span className="text-sm text-muted-foreground">Refreshing routes…</span>
              </div>
            </div>
          )}
        </div>
