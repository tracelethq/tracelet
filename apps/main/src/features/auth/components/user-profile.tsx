...
export function UserProfile({ collapsed }: { collapsed: boolean }) {
  ...
  useEffect(() => {
    if (!isPending && error) router.push(AUTH_ROUTES.signIn);
  ...
  function handleSignOut() {
    ...
    signOut({
      fetchOptions: {
        onSuccess: () => router.push(AUTH_ROUTES.signIn),
      },
    });
  }
    ...
    <DropdownMenu>
        ...
        >
          ...
          {!collapsed && (
            <span className="min-w-0 flex-1 truncate">
              ...
              {email && (
                <span className="block truncate text-xs text-sidebar-foreground/80">{email}</span>
              )}
            </span>
          )}
        </button>
      ...
      <DropdownMenuContent align="start" side="right" className="w-56">
        <DropdownMenuItem asChild>
          <Link href={APP_ROUTES.profile.route} className="gap-2">
