...
sections:
  - id: what-is-meta
    label: What is meta?
  - id: providing-meta
    label: Providing meta (code vs file)
  - id: tracelet-doc-json
    label: Configuring tracelet.doc.json
  - id: schema-reference
    label: Schema reference
  - id: merging
    label: How meta is merged
  - id: full-example
    label: Full example
---
...
### TraceletProperty (one field in request/query/params or inside a response)

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | `string` | Yes | Property name. |
| `type` | `string` | Yes | Type name: e.g. `"string"`, `"number"`, `"boolean"`, `"object"`, `"array"`, `"enum"`. |
| `desc` | `string` | No | Human-readable description. |
| `required` | `boolean` | No | Whether the field is required. |
| `enum` | `string[]` | No | When `type` is `"enum"`, the list of allowed values. |

### TraceletResponseProperty (one response variant)
