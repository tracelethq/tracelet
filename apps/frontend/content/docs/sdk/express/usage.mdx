...
You can provide meta in two ways: **in code** (pass the `meta` option to `tracelet({ app, meta })`) or **in a JSON file** (by default `tracelet.doc.json`, configurable via `traceletDocOptions.docFilePath`). You can use both; entries for the same method + path are merged, with **code meta overriding** file meta.
...
| Source | When to use |
|--------|-------------|
| **In code** | You define routes in Express and want a single source of truth. Build the meta array next to your routes (or derive it from your router) and pass it as the `meta` option to `tracelet({ app, meta })`. |
| **tracelet.doc.json** | You want to document APIs without touching application code, or you generate the file from OpenAPI/Swagger, another tool, or a build step. Set `traceletDocOptions.docFilePath` (default `tracelet.doc.json`). The file is loaded from disk when the server starts. |
...
To use a different path, set `traceletDocOptions.docFilePath` in your `tracelet()` call:
...
- If the file is missing or invalid JSON, Tracelet skips it and uses only the `meta` option passed to `tracelet()` (if any).
...
Resolved paths: `/users`, `/users/:id`, `/users` (POST). In code you can build the same structure and pass it as the `meta` option to `tracelet({ app, meta })`.

---

## How meta is merged

When you use both a **file** (`traceletDocOptions.docFilePath`) and the **meta** option:

1. Tracelet loads the array from the doc file (if the file exists and is valid).
2. It merges with the `meta` array passed to `tracelet({ app, meta })`.
...
5. The resulting array is what the UI and the route-meta endpoint (e.g. `GET /tracelet-docs?json=true`) use.
